...
static inline void Multiply512(DWORD Input, DWORD& lowResult,
                               DWORD& highResult)
{
  lowResult = Input << 9;
...
static HANDLE Get_WinNT_Disk(unsigned DriveIndex) {
  char DiskPath[] = "\\\\.\\A:";
  DiskPath[4] = 'A' + DriveIndex;
  return CreateFile(DiskPath, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
                              NULL, OPEN_EXISTING, 0, NULL);
}

static bool Win95_ReadSectors(HANDLE vwin32Device, unsigned DriveIndex,
...
static bool WinNT_ReadSectors(HANDLE DiskDevice, void *Buffer,
                              ...
                              std::size_t cSectorsToRead)
{
  ...
  if (static_cast<DWORD>(-1) ==
      SetFilePointer(DiskDevice, ByteIndex_low, &lByteIndex_high, FILE_BEGIN)
...
unsigned Disk::GetDriveIndex() const {
  return DriveIndex;
}
...
bool Disk::Valid() const {
  return Win95_vwin32.Valid() || WinNT_Disk.Valid();
}

bool Disk::ReadSectors(void *Buffer, unsigned long SectorIndex,
